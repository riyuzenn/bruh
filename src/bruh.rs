use dialoguer::{
    Select,
    Input,
    Confirm,
    theme::ColorfulTheme,
    console::Term
};

use std::{
    process::{Command, exit}, 
    io::Result,
    path::Path,
};

pub struct BruhCommit {
    title: String,
    items: Vec<String>
}


#[allow(unused_assignments)] // why not?
impl BruhCommit {
    pub fn new(title: &str, i: Vec<String>) -> BruhCommit {
        BruhCommit { 
            title: title.to_string(),
            items: i
        }
    }

    
    fn ensure_git(&self) {
        let g = Path::new(".git");
        if !g.exists() {
            println!("Not a git repository...");
            exit(1);
        }
        Command::new("git")
            .args(["add", "."])
            .output()
            .expect("Failed to execute the command");

    }
    fn capitalize(&self, s: &str) -> String {
        let mut c = s.chars();
        match c.next() {
            None => String::new(),
            Some(f) => f.to_uppercase().collect::<String>() + c.as_str(),
        }
    }

    fn get_commit_type(&self, default: usize) -> Result<usize> {
        let mut r#type: usize = 0;
        let sel = Select::with_theme(&ColorfulTheme::default())    
            .items(&self.items)
            .with_prompt(format!("{}", self.title))
            .default(default)
            .interact_on_opt(&Term::stderr());
        
        match sel.unwrap() {
            Some(i) => r#type = i,
            None => {
                println!("No selection is made. Please make sure to select a commit type");
                exit(1);
            } 
        }

        Ok(r#type)

    }
    
    fn get_commit_msg(&self) -> String {
        Input::with_theme(&ColorfulTheme::default())
            .with_prompt("Enter commit message")
            .interact_text().unwrap()
        
    }

    /// Execute the all needed params such as commit type
    /// and commit message
    ///
    pub fn exec(&self) -> Result<()> {
        // Check if the git is initialized then execute git add .
        self.ensure_git();

        let commit_type = self.get_commit_type(0)?;
        let commit_msg = self.get_commit_msg();
        
        let commit = format!(
            "{}: {}", 
                self.items[commit_type].split(":")
                    .collect::<Vec<&str>>()[0], 
                self.capitalize(commit_msg.as_str())
        );
        
        let cmd = Command::new("git")
            .args(["commit", "-m", &commit])
            .output()
            .expect("Failed to execute the command.");

        println!("\n{}", String::from_utf8(cmd.stdout).unwrap());

        let push = Confirm::with_theme(&ColorfulTheme::default())
            .with_prompt("Do you want to push?")
            .interact()?;
        
        if push {
            let cmd = Command::new("git")
                .arg("push")
                .output()
                .expect("Failed to execute the command");

            if cmd.stderr.is_empty() {
                println!("{}", String::from_utf8(cmd.stderr).unwrap());
            } else {
                println!("{}", String::from_utf8(cmd.stdout).unwrap());
            }
        } 

        Ok(())    
    }
}


